...
Analytics gebruikt Google Consent Mode v2 in de geavanceerde variant. De externe Google-tag staat één keer, direct bovenaan in de `<head>` van elke productiepagina. Alle opslag- en advertentiesignalen staan vóór de tag standaard op `denied`; alleen `analytics_storage` wordt na “Analytics toestaan” bijgewerkt naar `granted`. Zonder toestemming kan Google beperkte, cookieloze signalen ontvangen, maar worden geen Analytics-cookies geplaatst. Controleer in Tag Assistant zowel de eerste geweigerde consentgebeurtenis als de update na toestemming.
